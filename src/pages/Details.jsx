...
const Details = () => {
    ...
    const [eliminandoDepartamento, setEliminandoDepartamento] = useState(false);
    ...
    const eliminarDepartamento = async () => {
        if (!isArrendatario || !departamento?._id || eliminandoDepartamento) return;
        ...
        setEliminandoDepartamento(true);
        const loadingToast = toast.loading("Eliminando departamento...");

        try {
            const storedUser = JSON.parse(localStorage.getItem("auth-token"));
            const url = `${import.meta.env.VITE_BACKEND_URL}/departamento/eliminar/${departamento._id}`;
            ...
            setEliminandoDepartamento(false);
        }
    };
                        ...
                        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            {isArrendatario && (
                                <button
                                    ...
                                    disabled={eliminandoDepartamento}
                                    className="inline-flex items-center justify-center rounded-lg border border-red-300 px-4 py-2 text-sm font-semibold text-red-700 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60"
                                >
                                    {eliminandoDepartamento ? "Eliminando..." : "Eliminar departamento"}
                                </button>
                            )}
                        </div>
