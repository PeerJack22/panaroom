...
const Chat = () => {
    ...
    useEffect(() => {
        ...
        newSocket.on("nuevo-mensaje-chat", (mensaje) => {
    ...
    const enviarMensaje = async (data) => {
        ...
        try {
            const payload = {
                ...
                remitente: rol,
                administradorId: isAdmin ? userId : (contactoActivo.tipo === "administrador" ? contactoActivo.id : null),
                arrendatarioId: isArrendatario ? userId : (contactoActivo.tipo === "arrendatario" ? contactoActivo.id : null),
                estudianteId: isEstudiante ? userId : (contactoActivo.tipo === "estudiante" ? contactoActivo.id : null),
            };

            console.log("[Chat] Enviando mensaje:", payload);

            // Aquí irá la llamada al endpoint si es necesario
            // Por ahora emitimos por socket
                    ...
                    <div className="border border-gray-200 rounded-lg p-4 bg-gray-50 flex flex-col">
                        ...
                        {!contactoActivo ? (
                            ...
                            <div className="space-y-2 flex-1 overflow-y-auto">
                                <div
                                    onClick={() => setContactoActivo(contactoActivo)}
                                    className="p-3 rounded-lg bg-white border-2 border-blue-600 cursor-pointer hover:bg-blue-50 transition-colors"
                                >
