...
const FormularioPerfil = () => { 
    ...
    const updateUser = async (data) => { 
        ...
        Object.keys(data).forEach((key) => { 
            // Si la opción seleccionada es "upload" y el campo es "imagenPerfil" 
            if (key === "imagenPerfil" && data.profileImageOption === "upload") { 
                if (data.imagenPerfil[0]) { 
                    formData.append("imagenPerfil", data.imagenPerfil[0]); 
                } 
            }  
            // Si la opción seleccionada es "ia" 
            else if (key === "avatarProfileIA" && data.profileImageOption === "ia") { 
                formData.append(key, data[key]); 
            } else { 
                // Para el resto de los campos de texto 
                formData.append(key, data[key]); 
            } 
        }); 

        // Llama a la función `updateProfile` con el FormData 
    ...
    useEffect(() => { 
        if (user) { 
            reset({ 
                ...
                email: user?.email, 
                // Aquí puedes inicializar la opción de imagen si el usuario ya tiene una 
                // Por ejemplo, si tu `user` tiene un campo `avatarType` 
                profileImageOption: user?.avatarType || "upload", 
            }); 

            // Si el usuario tiene un avatar de IA, inicializa el estado 
            if (user?.avatarType === "ia" && user?.avatarUrl) { 
                setStateProfileAvatar((prev) => ({ 
                    ...prev, 
                    generatedImage: user.avatarUrl, 
                })); 
                setValue("avatarProfileIA", user.avatarUrl); // Puedes guardar la URL o el base64 si lo tienes 
            } 
